...
if(isset($_POST['login']))
{
  ...
  {
    header("Location: mycan_log.php?login%empty");
    exit();
  }
  else
  {
    $my_user = "SELECT * FROM candidate WHERE user_email='".$email."' ";
    ...
    if(mysqli_num_rows($query) > 0)
    {
      ...
      {
        $email     = htmlentities(mysqli_real_escape_string($con, $_POST['user_email']));
        $get_user  = "SELECT * FROM candidate WHERE user_email='".$email."' ";
        ...
        if ($user_auth == 1) 
        {
          ...
          $online     = "UPDATE candidate SET log_in='Online' WHERE user_email='".$email."' ";
          ...
          echo "<script>window.open('mycandidate.php?user_name%$user_name', '_self')</script>";
        }
        else 
        {
      ...
      {
        header("Location: mycan_log.php?login%error");
        exit();
      }
    }
    else
        ...
        <form action="" method="POST" autocomplete="off" class="sign">
          ...
          <div class="direction">
            ...
            <div class="direct_two">
              <a href="mycan_reg.php" class="log">Create new account</a>
            </div>
          </div>

          <div class="forgotPass">
            <div class="pwd_reset">
              <a href="mycan_Request.php" class="log">Forgot Password</a>
            </div>
          </div>
        </form>
      </div>
