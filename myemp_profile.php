    ...
    if(in_array($fileActualExt, $allowed)) 
    {
      if($fileError === 0) 
      {
        ...
        { 
          ...
          $fileDestination = 'emprofile/'.$fileNameNew;
          move_uploaded_file($fileTmpName, $fileDestination);

          $comp_name = htmlentities(mysqli_real_escape_string($con, $_POST['comp_name']));
          $started   = htmlentities(mysqli_real_escape_string($con, $_POST['started']));
          $services  = htmlentities(mysqli_real_escape_string($con, $_POST['services']));
          ...
          $owner     = htmlentities(mysqli_real_escape_string($con, $_POST['owner']));
          $qlfcation = htmlentities(mysqli_real_escape_string($con, $_POST['qlfcation']));
          $xp        = htmlentities(mysqli_real_escape_string($con, $_POST['xp']));
          $token     = $_POST['token'];

          if(empty($comp_name) || empty($started) || empty($services) || empty($location) 
          || empty($p_box) || empty($phone) || empty($email) || empty($owner) || 
          empty($qlfcation) || empty($xp))
          {
            echo "<script>alert('You cant submit an empty form')</script>";
            echo "<script>window.open('myemp_settings.php?token=$token', 
            '_self')</script>";
            exit();
          }
          else
          {
            if (!preg_match("/^[a-z A-Z]*$/", $owner))
            {
              echo "<script>alert('Your form had an issue, Please retry again')</script>";
              echo "<script>window.open('myemp_settings.php?token=$token', 
              '_self')</script>";
              exit();
            }
            else 
            {
              ...
              {
                echo "<script>alert('Your form had an issue, Please retry again')
                </script>";
                echo "<script>window.open('myemp_settings.php?token=$token', 
                '_self')</script>";
                exit();
              ...
              {
                $sql = "SELECT * FROM emprofile WHERE comp_name='$comp_name' OR 
                phone='$phone' OR email='$email' OR owner='$owner' OR token='$token'";
                $result = mysqli_query($con, $sql);
                $resultCheck = mysqli_num_rows($result);

                if($resultCheck > 0)
                {
                  echo "<script>alert('You have used cridentials that already exist, Please retry again')</script>";
                  echo "<script>window.open('myemp_settings.php?token=$token', 
                  '_self')</script>";
                  exit();  
                }
                ...
                {
                  $sqlquery = "INSERT INTO emprofile(photo,comp_name,started,services,
                  location,p_box,phone,email,owner,qlfcation,xp,token)
                  VALUES('$fileDestination','$comp_name','$started','$services',
                  '$location','$p_box','$phone','$email','$owner','$qlfcation','$xp',
                  '$token')";
                  ...
                  if ($verifyquery) 
                  {
                    echo "<script>alert('Your have completed your Profile form successfully')</script>";
                    echo "<script>window.open('myemp_settings.php?token=$token', 
                    '_self')</script>";
                    exit();
                  }
          ...
          echo "<script>window.open('myemp_settings.php?token=$token', '_self')</script>";
          exit();
        }
      }
      else {
      ...
      echo "<script>alert('You cant submit an empty form, Please complete your profile form')</script>";
      echo "<script>window.open('myemp_settings.php?token=$token', '_self')</script>";
      exit();
    }
  }
          ...
          <div class="layerthree">
            ...
            <div class="job_page">
              <div class="job_inner">
                ...
                <?php
                  $user        = $_SESSION['user_email'];
                  $get_profile = "SELECT * FROM emprofile WHERE email='$user'";
                ...
                id="form" autocomplete="off" style="padding-top:35px;">
                  ...
                  <div class="job_body">
                    ...
                    <div class="box">
                      <div class="inputWithIcon inputIconBg"> 
                        <input type="text" name="started" placeholder="When the company started" value="<?php echo $started ?>">
                    ...
                    <div class="box">
                      <div class="inputWithIcon inputIconBg"> 
                        <input type="text" name="services" placeholder="Services of the Company" value="<?php echo $services ?>">
                        <i class="fa fa-tasks fa-lg fa-fw" aria-hidden="true"
                        style="margin-top:2px"></i>
                      </div>
                    </div>
                    ...
                    <div class="box_one">
                      <div class="inputWithIcon inputIconBg">
                        <input type="text" name="p_box" placeholder="P.O Box"
                    ...
                    <div class="title"><h4>CEO/EXECUTIVE DIRECTOR</h4></div>

                    <div class="box_two">
                      <div class="inputWithIcon inputIconBg">
                        <input type="text" name="owner" placeholder="Full name"
                    ...
                    <div class="box_two">
                      <div class="inputWithIcon inputIconBg">
                        <input type="text" name="qlfcation" placeholder="Your Qualifications" value="<?php echo $qlfcation ?>">
                    ...
                    <div class="box_two">
                      <div class="inputWithIcon inputIconBg">
                        <input type="text" name="xp" placeholder="Your Experience"
                        value="<?php echo $xp ?>">
                    ...
                    <div class="finalise">
                      ...
                      <?php if(mysqli_num_rows($query) > 0) { ?>
                        <input type="hidden" class="form-control submit" value="save">
                      <?php } else { ?>
                        <input type="submit" class="form-control submit" name="save" 
                        value="save">
                      <?php } ?>  
                    </div>
                  </div>
                  ...
                  <div class="modal-body">
                    ...
                    id="form" autocomplete="off">
                      ...
                      <div class="job_body">
                        <div class="box_top">
                          <div class="inputWithIcon inputIconBg"> 
                            <input type="text" name="comp_name" value="<?=$comp_name?>">
                            <i class="fa fa-globe fa-lg fa-fw" aria-hidden="true"
                            style="margin-top:2px"></i>
                          </div>
                        </div>
                        ...
                        <div class="box">
                          <div class="inputWithIcon inputIconBg"> 
                            <input type="text" name="started" value="<?= $started ?>">
                            <i class="fa fa-clock-o fa-lg fa-fw" aria-hidden="true"
                            style="margin-top:2px"></i>
                          </div>
                        </div>
                        ...
                        <div class="box_one">
                          <div class="inputWithIcon inputIconBg">
                            <input type="text" name="phone" value="<?= '0'.$phone ?>">
                            <i class="fa fa-phone fa-lg fa-fw" aria-hidden="true"></i>
                          </div>
                        </div>
                        ...
                        <div class="box_two">
                          <div class="inputWithIcon inputIconBg">
                            <input type="text" name="qlfcation" value="<?=$qlfcation?>">
                            <i class="fa fa-file-text-o fa-lg fa-fw" aria-hidden="true"></i>
                          </div>
                        </div>
                        ...
                        <div class="finalise">
                          <input type="submit" class="form-control submit" name="change"
                          value="update">  
                        </div>
                      </div>
                    </form>
            ...
            <?php
              ...
              {     
                ...
                if(in_array($fileActualExt, $allowed)) 
                {
                  if($fileError === 0) 
                  {
                    ...
                    { 
                      ...
                      echo "<script>window.open('myemp_profile.php?token=$token', '_self')</script>";
                      exit();
                    }
                  }
                  else {
                ...
                $sqlupdt = "UPDATE emprofile SET photo='$fileDestination', 
                comp_name='$comp_name', started='$started', services='$services',
                location='$location', p_box='$p_box', phone='$phone', owner='$owner',
                qlfcation='$qlfcation', xp='$xp' WHERE token='$token' ";

                $resupdt = mysqli_query($con, $sqlupdt);

                if ($resupdt) {
                  $newUser    = $_GET["token"];
                  $getNewUser = "SELECT * FROM emprofile WHERE token='$newUser'";
                  ...
                  if(mysqli_num_rows($resNewUser) == False) { 
                    ...
                    $compName  = $main['comp_name'];
                    $location  = $main['location'];
                    $phone     = $main['phone'];
                  }

                  $forJob = $_SESSION['user_email'];
                  $resJob = "SELECT * FROM empostjob WHERE email='$forJob'";
                  ...
                  if(mysqli_num_rows($runJob) == False) { 
                    ...
                    $oldPhoto    = $sub['photo']; 
                    $oldCompName = $sub['comp_name'];
                    $oldLocation = $sub['location'];
                    $oldPhone    = $sub['phone'];

                    if($mainPhoto == $oldPhoto && $compName == $oldCompName && 
                    $location == $oldLocation && $phone == $oldPhone) {
                      // leave blank
                    }
                    else {
                      $userUnify = $_SESSION['user_email'];
                      $getUnify  = "SELECT * FROM empostjob WHERE email='$userUnify'";
                      ...
                      $update = "UPDATE empostjob SET photo='$newphoto',
                      comp_name='$newcompname',location='$newlocation',
                      phone='$newphone' WHERE email='$userUnify'";
                      $run_update = mysqli_query($con, $update);

                      echo "<script>alert('Thank you for updating your Profile form. In this case if you haven't selected a new photo you will need to select the old one ')</script>";
                      echo "<script>window.open('myemp_profile.php?token=$token', '_self')</script>";
                      exit();
                    }
                  }
