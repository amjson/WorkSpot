...
if(isset($_POST['signup']))
{
  ...
  $firstname  = htmlentities(mysqli_real_escape_string($con, $_POST['user_firstname']));
  $lastname   = htmlentities(mysqli_real_escape_string($con, $_POST['user_lastname']));
  ...
  $proffesion = htmlentities(mysqli_real_escape_string($con, $_POST['user_proffesion']));
  ...
  if(empty($firstname) || empty($lastname) || empty($username) || empty($phone) 
  || empty($email) || empty($proffesion) || empty($gender) || empty($password) 
  || empty($confirm))
  {
    header("Location: mycan_reg.php?signup%empty");
    exit();
  }
  else
  {
    ...
    {
      header("Location: mycan_reg.php?signup%error");
      exit(); 
    }
    else 
    {
      ...
      {
        $sql = "SELECT * FROM candidate WHERE user_name='$username' OR user_email='$email'
        ...
        if($resultCheck > 0)
        {
          header("Location: mycan_reg.php?signup%credentials%taken");
          exit();     
        }
        else 
        {
          ...
          if($password === $confirm)
          {
            ...
            {
              header("Location: mycan_reg.php?signup%atleast%9%characters%password");
              exit();
            }
            else
            {
              ...
              $sqlquery = "INSERT INTO candidate(user_firstname,user_lastname,user_name,
              user_phonenumber,user_email,user_proffesion,user_gender,acc_verify,token,
              user_pass,user_profile,date_created)
              VALUES('$firstname','$lastname','$username','$phone','$email','$proffesion',
              '$gender','1','$token','$hashedPass','$profile_pic',now())";
              ...
              if ($verifyquery) 
              {
                ...
                try  
                {
                  ...
                  // Content
                  $url = "http://" . $_SERVER["HTTP_HOST"] . dirname($_SERVER["PHP_SELF"]) . "/includes/verify_can.php?token=$token";
                ...
                catch (Exception $e)
                {
                  ...
                  echo "<script>window.open('mycan_reg.php', '_self')</script>";
                  exit();
                }  
                exit();
              }
          ...
          {
            echo "<script>window.open('mycan_reg.php?signup%error', '_self')</script>";
          }
        }
      }
    }
        ...
        <form action="" method="POST" autocomplete="off" class="sign">
          <div class="banner-body">
            ...
            <div class="middleSecondLeft">
              <div class="inputWithIcon inputIconBg"> 
                <input type="text" name="user_proffesion" placeholder="Proffesion">
            ...
            <div class="middleSecondRight">
              <div class="inputWithIcon inputIconBg"> 
                <select type="text" name="user_gender">
                  <option disabled="">select your gender</option>
                  <option style="color: #000;">Male</option>
                  <option style="color: #000;">Female</option>
                </select>
                <i class="fa fa-venus fa-lg fa-fw" aria-hidden="true" style="margin-top:4px;
                margin-left:3px;"></i>
              </div>
            </div>
          ...
          <div class="direction">
            ...
            <div class="direct_two">
              <a href="mycan_log.php" class="log">Already a member</a>
            </div>
          </div>
        </form>
      </div>
