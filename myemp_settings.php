          ...
          <div class="layerthree">
            <div class="job_cover">
              ...
              margin-top:86px;margin-left:70px;">
                <?php
                  if (isset($_POST['forgeneral'])) {
                    $company  = htmlentities($_POST['forComp']);   
                    $fullname = htmlentities($_POST['forName']);
                    $username = htmlentities($_POST['forUname']);
                    $phone    = htmlentities($_POST['forPhone']);
                    $email    = htmlentities($_POST['forEmail']);
                    $location = htmlentities($_POST['forLocation']);

                    $update = "UPDATE employer SET user_company='$company', 
                    user_fullname='$fullname',user_name='$username',
                    user_phonenumber='$phone',user_email='$email',
                    user_location='$location' WHERE user_email='$user'";
                    ...
                    if ($run) {
                      ...
                      $resProfile = "SELECT * FROM emprofile WHERE token='$forProfile'";
                      ...
                      if(mysqli_num_rows($runProfile) == False) { 
                        ...
                        if($mainMail == $oldEmail) {
                          ...
                          $mytoken  = $_GET["token"];
                          $resToken = "SELECT * FROM emprofile WHERE token='$mytoken'";
                          ...
                          $update  ="UPDATE emprofile SET email='$newMail' 
                          WHERE token='$mytoken'";
                          $runUPDT = mysqli_query($con, $update);

                          $newUserTwo   =$_GET["token"];
                          $getNewUserTwo="SELECT * FROM employer WHERE token='$newUserTwo'";
                          ...
                          if(mysqli_num_rows($runJob) == False) { 
                            ...
                            if($mainMailTwo == $oldEmailTwo) {
                              ...
                              $resTokenTwo = "SELECT * FROM empostjob WHERE 
                              reg_token='$mytokenTwo'";
                              ...
                              // updating child directory with new names
                              $updateTwo ="UPDATE empostjob SET email='$newMailTwo' 
                              WHERE reg_token='$mytokenTwo'";
                ...
                <div class="boxOne">
                  <form action="" method="post" autocomplete="off" id="form">
                    <?php
                      $user        = $_SESSION['user_email'];
                      $get_profile = "SELECT * FROM employer WHERE user_email='$user'";
                      $run_profile = mysqli_query($con, $get_profile);
                      $row         = mysqli_fetch_array($run_profile);

                      $photo       = $row['user_profile'];
              ...
              margin-top:-435px;margin-right:70px;float:right;">
                    ...
                    if(in_array($fileActualExt, $allowed)) {
                      if($fileError === 0) {
                        if($fileSize < 10000000) {
                          ...
                          $update = "UPDATE employer SET user_profile=
                          '$fileDestination' WHERE user_email='$user'";
                          $run    = mysqli_query($con, $update);

                          if ($run) {
                            echo "<script>alert('Your Profile Updated successfully')</script>";
                            echo "<script>window.open('myemp_settings.php', '_self')</script>";
                            exit();
                          }
                        }
                        else {
                ...
                <div class="boxTwo">
                  <div class='cardphoto'><img src='<?php echo $photo; ?>'></div>
                  <div class='cardname'><?php echo $user_fullname; ?></div>
                  <div class='cardcompany'><?php echo $user_company; ?></div>
                ...
                <?php 
                  if(isset($_POST['forpassword'])) {
                    ...
                    if(empty($c_pass) || empty($pass1) || empty($pass2)) {
                      echo "<script>window.open('myemp_settings.php?form%empty', '_self')</script>";
                      exit();
                    } 
                    else {
                      ...
                      if(password_verify($c_pass, $password_hash)) {
                        if ($pass1 === $pass2) {
                          if(strlen($pass1) > 9) {
                            ...
                            $update_pass = "UPDATE employer SET user_pass=
                            ...
                            echo "<script>window.open('myemp_settings.php?form%atleast%9%characters%password', '_self')</script>";
                          }
                        }
                        else {
                          echo "<script>window.open('myemp_settings.php?form%password%dont%match', '_self')</script>";
                        }
                      }
                      else {
                        echo "<script>window.open('myemp_settings.php?form%invalid%password', '_self')</script>";
                      }
                    }
                  }
                ?>
